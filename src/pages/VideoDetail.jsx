...
import { getVideoBySlug, getRelatedVideos, getVideoNeighbors } from '@/data/videos';
...
import { ArrowLeft, ChevronLeft, ChevronRight, ExternalLink } from '@/components/ui/icons';
...
function Credits({ credits }) {
  ...
  return (
    <div>
      <h2 className="text-sm font-semibold uppercase tracking-[0.16em] text-bone-muted">Credits</h2>
...
function VideoNotFound() {
  return (
    <Container className="pb-24 pt-40 text-center">
      ...
      <h1 className="mt-4 font-display text-4xl font-bold text-bone">Video not found</h1>
...
export default function VideoDetail() {
  ...
  const aside = imageFile
    ? getImageSource(imageFile, { title: video.title, label: video.category, seed: video.slug })
    : null;
  const meta = [video.year, isFilm ? video.genre : video.category].filter(Boolean).join(' · ');
    ...
    <article className="pt-28 sm:pt-32">
      <Container>
        ...
        <ScrollReveal className="mt-6">
          ...
          <h1 className="mt-3 max-w-4xl font-display text-4xl font-bold leading-[0.95] tracking-tightest text-bone sm:text-6xl lg:text-7xl">
            {video.title}
          </h1>
        </ScrollReveal>
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-3">
          <ScrollReveal className="flex flex-col gap-10 lg:col-span-2">
            {video.description && <p className="prose-cine text-lg">{video.description}</p>}
            ...
            {aside && (
              <div className="overflow-hidden rounded-2xl border border-white/10 bg-ink-850">
                <img
                  ...
                  loading="lazy"
                  decoding="async"
                  className={cn('w-full object-cover', isFilm ? 'aspect-[2/3]' : 'aspect-square')}
                />
              </div>
            )}
            <Button
        ...
        <nav aria-label="Video navigation" className="mt-14 grid gap-4 sm:grid-cols-2">
